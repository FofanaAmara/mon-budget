...
import { calcMonthlyIncome, formatShortDate, getNextBiweeklyPayDate } from '@/lib/utils';
 ...
 */
const SOURCE_META: Record<IncomeSource, { label: string; emoji: string; bg: string; cssClass: string }> = {
  EMPLOYMENT: { label: 'Emploi',         emoji: '💼', bg: '#EFF6FF', cssClass: 'emploi' },
  BUSINESS:   { label: 'Business',       emoji: '🏢', bg: '#F0FDFA', cssClass: 'business' },
  INVESTMENT: { label: 'Investissement', emoji: '📈', bg: '#ECFDF5', cssClass: 'investissement' },
...
function formatMonthlyDisplay(amount: number): string {
  return amount.toLocaleString('fr-CA', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
}
    ...
    >
        ...
        {menuOpen && (
          <>
            ...
            <div style={{
              ...
              <button
                onClick={() => { setMenuOpen(false); onEdit(inc); }}
                style={{
                  width: '100%',
                  padding: '10px 16px',
                  display: 'flex',
              ...
              <button
                onClick={() => { setMenuOpen(false); onDelete(inc.id); }}
                style={{
                  width: '100%',
                  padding: '10px 16px',
                  display: 'flex',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
        ...
        <div style={{
          ...
          {isVariable ? '~' : ''}{formatMonthlyDisplay(monthly)}
          <span style={{ fontSize: '0.5em', fontWeight: 600, color: 'var(--teal-700)' }}>$</span>
          <span style={{ fontSize: '0.4em', fontWeight: 500, color: 'var(--slate-400)', letterSpacing: 0 }}>/mois</span>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="14" height="14" style={{ color: 'var(--slate-400)', flexShrink: 0 }}>
  ...
  return (
    <div style={{ textAlign: 'center', padding: '48px 20px' }}>
      ...
      <p style={{
        ...
        Ajoute ta premiere source de revenu pour suivre tes entrees d&apos;argent.
      </p>
      <button
        onClick={onAdd}
        style={{
