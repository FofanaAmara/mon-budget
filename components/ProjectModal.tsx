...
export default function ProjectModal({ sections, onClose }: Props) {
  ...
  const monthlySuggested = targetAmount && targetDate
    ? calcMonthlySuggested(parseFloat(targetAmount), parseFloat(initialAmount || '0'), targetDate)
    : null;

  const monthsRemaining = targetDate
    ? Math.max(0, Math.ceil((new Date(targetDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24 * 30.44)))
    : null;
    ...
    <div className="sheet-backdrop" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: '20px 24px 24px' }}>
          <form id="project-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
            ...
            {monthlySuggested !== null && monthlySuggested > 0 && (
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '3px 0' }}>
                  <span style={{ fontSize: '13px', fontWeight: 500, color: 'var(--slate-500)' }}>Contribution suggérée</span>
                  <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--teal-700)', letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums' }}>
                    ~{Math.ceil(monthlySuggested).toLocaleString('fr-CA')} $ / mois
                  </span>
                </div>
                {monthsRemaining !== null && (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '3px 0' }}>
                    <span style={{ fontSize: '13px', fontWeight: 500, color: 'var(--slate-500)' }}>Temps restant</span>
