...
export default function IncomeModal({ income, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isVariable && !amount) { setError('Le montant est requis pour un revenu fixe'); return; }
    setLoading(true);
    setError('');
    try {
      const data = {
  ...
  return (
    <>
      ...
      <div className="income-modal-container" onClick={(e) => e.stopPropagation()}>
        ...
        >
          <span style={{ width: '36px', height: '4px', borderRadius: '2px', background: 'var(--slate-300)', display: 'block' }} />
        </div>

        {/* ── Header ── */}
        <div className="income-modal-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 20px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{
              ...
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="20" height="20" style={{ color: 'var(--teal-700)' }}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="income-modal-body" style={{ padding: '20px 20px 0' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {SOURCE_OPTIONS.map((opt) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ fontSize: '16px', lineHeight: 1 }}>{opt.emoji}</span>
                      {opt.label}
                    </button>
                  );
                })}
            ...
            {isVariable && (
              <div style={{
                ...
                Ce revenu varie d&apos;un mois a l&apos;autre. Le montant estime sert pour tes previsions.
              </div>
            )}
            ...
            {isVariable && (
              <div style={{ marginBottom: '18px' }}>
                <label style={labelStyle}>Montant estime (pour les previsions)</label>
                <div style={{ position: 'relative' }}>
                  <span style={{
                    position: 'absolute',
                    left: '14px',
                ...
                <p style={{ fontSize: '12px', fontWeight: 600, color: 'var(--amber-600)', marginTop: '6px', letterSpacing: '-0.01em' }}>
            ...
            {showAnchorDate && (
              <div style={{ marginBottom: '18px' }}>
                <label style={labelStyle}>Date d&apos;ancrage du paiement</label>
                <div style={{ position: 'relative' }}>
                  <input
                    type="date"
                    className="income-modal-input"
                  ...
                  />
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" width="18" height="18" style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', color: 'var(--slate-400)', pointerEvents: 'none' }}>
                ...
                <p style={{ fontSize: '12px', fontWeight: 500, color: 'var(--slate-400)', marginTop: '6px', letterSpacing: '-0.01em' }}>
                  Date d&apos;une prochaine paie — la suivante sera calculee automatiquement
                </p>
              </div>
            )}
              ...
              <div style={{ marginBottom: '18px' }}>
                <div style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '12px', fontWeight: 400, color: 'var(--slate-500)', marginTop: '2px', lineHeight: 1.4 }}>
                      Marquer le revenu comme recu sans action de ta part
                    </div>
                  </div>
                  <label style={{ position: 'relative', width: '48px', height: '28px', flexShrink: 0 }}>
                    <input
                      type="checkbox"
                      checked={autoDeposit}
                    ...
                    <span
                      ...
                      style={{
                        position: 'absolute',
                        inset: 0,
                        background: autoDeposit ? 'var(--teal-700)' : 'var(--slate-300)',
                        borderRadius: '14px',
            ...
            <div style={{ marginBottom: '4px' }}>
              <label style={labelStyle}>
                Notes{' '}
                <span style={{ fontWeight: 500, textTransform: 'none', letterSpacing: 0, color: 'var(--slate-400)' }}>
                  (optionnel)
                </span>
              </label>
              <textarea
          ...
          >
            <button
              ...
              onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--slate-50)'; e.currentTarget.style.borderColor = 'var(--slate-300)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = '#FFFFFF'; e.currentTarget.style.borderColor = 'var(--slate-200)'; }}
            >
              Annuler
            </button>
            <button
              ...
              onMouseLeave={(e) => {
                e.currentTarget.style.background = loading ? 'var(--slate-300)' : 'var(--teal-700)';
