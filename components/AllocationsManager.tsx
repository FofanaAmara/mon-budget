...
export default function AllocationsManager({
  ...
  >(undefined);
  const [deleteTarget, setDeleteTarget] = useState<IncomeAllocation | null>(
    null,
  );
      ...
      <div className="card" style={{ padding: "20px", marginBottom: "16px" }}>
        ...
        >
          <div style={{ textAlign: "center", flex: 1 }}>
            ...
            <p
              className="amount"
              style={{ fontSize: "var(--text-lg)", fontWeight: 750 }}
            >
        ...
        {isOverAllocated && (
          ...
          >
            ⚠ Surallocation de {formatCAD(Math.abs(disponible))} — le total
            alloué dépasse le revenu mensuel
          </div>
        )}
      </div>
            ...
            [
              {
                key: "charges",
                label: "📦 Charges",
                filter: (a: IncomeAllocation) => a.section_ids.length > 0,
              },
              {
                key: "savings",
                label: "💰 Épargne",
                filter: (a: IncomeAllocation) =>
                  a.section_ids.length === 0 && !!a.project_id,
              },
              {
                key: "free",
                label: "🔖 Autre",
              ...
              key: string;
              label: string;
              filter: (a: IncomeAllocation) => boolean;
            }[]
          ).map((group) => {
              ...
              <div key={group.key} style={{ marginBottom: "20px" }}>
                ...
                <div className="card" style={{ overflow: "hidden" }}>
                  {groupItems.map((alloc, i) => {
                    ...
                    const globalIndex = allocations.findIndex(
                      (a) => a.id === alloc.id,
                    );
                    return (
                      <div
                          ...
                          >
                            ...
                            <button
                              ...
                              style={{
                                padding: "4px",
                                color:
                                  globalIndex === allocations.length - 1
                                    ? "var(--text-tertiary)"
                                    : "var(--text-secondary)",
                                opacity:
                                  globalIndex === allocations.length - 1
        ...
        >
          <div className="sheet" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: "8px 24px 32px" }}>
              ...
              >
                "{deleteTarget.label}" sera retirée de toutes les allocations
                futures. Cette action est irréversible.
              </p>
