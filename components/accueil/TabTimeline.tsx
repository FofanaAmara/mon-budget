...
// Icon SVG by variant
function EventIcon({ variant }: { variant: TimelineIconVariant }) {
      ...
      {groups.map((group) => (
        <div key={group.date} style={{ position: "relative" }}>
          ...
          {group.events.map((ev, i) => {
            const variant = getTimelineIconVariant(ev);
            const badge = getBadgeVariant(ev);
            const badgeCfg = BADGE_STYLES[badge];
            const amountColor = getTimelineAmountColor(ev);
                ...
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  ...
                  >
                    {isFutureDate
                      ? "Prévu"
                      : ev.paidAt
                        ? new Date(ev.paidAt).toLocaleTimeString("fr-CA", {
                            hour: "2-digit",
                            minute: "2-digit",
                          })
                        : ""}
