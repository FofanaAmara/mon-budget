...
function CardIcon({
  ...
  className: "revenus" | "depenses" | "epargne" | "dettes";
}) {
...
export default function TabTableauDeBord({
  ...
  const savingsGoalTotal = savingsSummary.byProject.reduce(
    (s, p) => s + p.total,
    0,
  );
  const savingsPct =
    totalEpargne > 0 && savingsGoalTotal > 0
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: "3px",
                    background:
                      "linear-gradient(90deg, var(--positive), #10B981)",
                    width: `${savingsPct}%`,
