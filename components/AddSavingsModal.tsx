...
export default function AddSavingsModal({ expenseId, projectName, currentSaved, targetAmount, onDone, onClose }: Props) {
  ...
  const afterBalance = amount && parseFloat(amount) > 0 ? currentSaved + parseFloat(amount) : null;
  const progressAfter = targetAmount && afterBalance ? Math.min((afterBalance / targetAmount) * 100, 100) : null;
    ...
    <div className="sheet-backdrop" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: '20px 24px 24px' }}>
          <form id="savings-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
            ...
            <div style={{ marginBottom: '18px' }}>
              ...
              <p style={{ fontSize: '12px', fontWeight: 500, color: 'var(--slate-400)', marginTop: '6px' }}>
                Solde actuel du pot : <strong style={{ fontWeight: 700, color: 'var(--slate-700)' }}>{formatCAD(currentSaved)}</strong>
              </p>
            </div>
            ...
            {afterBalance !== null && (
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '3px 0' }}>
                  <span style={{ fontSize: '13px', fontWeight: 500, color: 'var(--slate-500)' }}>Après contribution</span>
                ...
                {targetAmount !== null && progressAfter !== null && (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '3px 0' }}>
                    <span style={{ fontSize: '13px', fontWeight: 500, color: 'var(--slate-500)' }}>Progression</span>
                    <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--slate-900)', letterSpacing: '-0.02em' }}>
                      {Math.round((currentSaved / targetAmount) * 100)}% → {Math.round(progressAfter)}%
                    </span>
                  </div>
                )}
              </div>
        ...
        <div style={{ display: 'flex', gap: '10px', padding: '0 24px 24px' }}>
          ...
          >
            {loading ? 'Enregistrement...' : `Ajouter${amount && parseFloat(amount) > 0 ? ` ${formatCAD(parseFloat(amount))}` : ''}`}
          </button>
        </div>
      </div>
    </div>
