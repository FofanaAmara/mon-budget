...
export default function TransferSavingsModal({ source, allPots, onDone, onClose }: Props) {
  ...
  const sourceAfter = amount && parseFloat(amount) > 0 ? sourceBalance - parseFloat(amount) : null;
  const destAfter = amount && parseFloat(amount) > 0 ? destBalance + parseFloat(amount) : null;
    ...
    <div className="sheet-backdrop" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="sheet" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: '20px 24px 24px' }}>
          <form id="transfer-form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
            ...
            <div style={{ marginBottom: '0' }}>
              ...
              <div style={{
                ...
                fontSize: '15px', fontWeight: 500, color: 'var(--slate-900)',
                background: 'var(--slate-50)',
              }}>
                {source.name}
                <span style={{ color: 'var(--slate-400)', fontWeight: 400 }}> ({formatCAD(sourceBalance)})</span>
              </div>
            </div>

            {/* Transfer arrow */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 0', color: 'var(--slate-300)' }}>
            ...
            <div style={{ marginBottom: '18px' }}>
              ...
              <p style={{ fontSize: '12px', fontWeight: 500, color: 'var(--slate-400)', marginTop: '6px' }}>
                Disponible dans {source.name} : <strong style={{ fontWeight: 700, color: 'var(--slate-700)' }}>{formatCAD(sourceBalance)}</strong>
              </p>
              {sourceBalance > 0 && (
                <button
                  type="button"
            ...
            {sourceAfter !== null && destAfter !== null && (
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '3px 0' }}>
                  <span style={{ fontSize: '13px', fontWeight: 500, color: 'var(--slate-500)' }}>{source.name} après</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '3px 0' }}>
                  <span style={{ fontSize: '13px', fontWeight: 500, color: 'var(--slate-500)' }}>{dest?.name} après</span>
                  <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--teal-700)', letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums' }}>
                    {formatCAD(destAfter)}
                  </span>
                </div>
              </div>
        ...
        <div style={{ display: 'flex', gap: '10px', padding: '0 24px 24px' }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 'var(--radius-md)', background: 'var(--teal-700)',
              fontFamily: 'var(--font)', fontSize: '15px', fontWeight: 700,
              color: 'var(--white, #fff)',
              cursor: loading || !destId || parseFloat(amount) <= 0 || parseFloat(amount) > sourceBalance ? 'not-allowed' : 'pointer',
              opacity: loading || !destId || parseFloat(amount) <= 0 || parseFloat(amount) > sourceBalance ? 0.5 : 1,
              transition: 'all 0.2s ease',
            }}
          >
            {loading ? 'Transfert...' : `Transférer${amount && parseFloat(amount) > 0 ? ` ${formatCAD(parseFloat(amount))}` : ''}`}
          </button>
        </div>
      </div>
    </div>
